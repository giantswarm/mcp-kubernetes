...
* The `helm.sh/chart` label no longer ends with an invalid character when the 63-character truncation of `<name>-<version>` lands on a `.` or `_`. Long dev chart versions previously made every labeled resource fail admission (`metadata.labels: Invalid value`), so the chart could not be installed.
...
* External-issuer tokens are impersonated when they resolve a validated human subject (an `email` claim matching `allowedClaims.email`). `Impersonate-User` is set to the human subject and `Impersonate-Group` to `system:authenticated`; no `Impersonate-Extra-*` headers are sent. Both an on-behalf-of token (an agent acting for a human, carrying an RFC 8693 `act` claim) and a human-direct token (a human reaching kubernetes tools through muster, no `act` claim) are accepted. An `act` claim is validated when present but is not required. A token with no human subject (a bare machine or ServiceAccount token) is rejected (403).

### Removed

* M2M (machine-to-machine) trusted-issuer support. The `trustedIssuers[].impersonateUser`, `impersonateGroups`, `alias`, and `allowedActors` (with per-actor `allowedSubjects`) fields are removed, along with the per-alias Namespace, `impersonate serviceaccounts` Role/RoleBinding, and per-alias ClusterRole. A trusted-issuer token is accepted only when it resolves a validated human subject; a bare machine or ServiceAccount token (no `email` claim) is rejected (403). Any validated trusted-issuer actor is allowed (the impersonated human's Kubernetes RBAC governs access). The per-issuer impersonation RBAC is consolidated into a single `*-obo-impersonate` ClusterRole granting `impersonate users` and `impersonate groups` (`system:authenticated`) only; no `userextras/*` grant.
