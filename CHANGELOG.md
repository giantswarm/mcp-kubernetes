...
- Helm: per trusted-issuer `kagent-<alias>` Namespace, namespaced Role granting `impersonate serviceaccounts` in that namespace, and a ClusterRole granting `impersonate groups` (restricted to `system:serviceaccounts:kagent-<alias>`) and `impersonate userextras/agent` + `userextras/issuer`. Generated when `serviceAccount.create` and `rbac.create` are both true and the issuer has an `alias` set.
