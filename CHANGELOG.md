...
- Adopt `github.com/giantswarm/mcp-toolkit` v0.1.0 partial: the toolkit's `responsecap` (default 128 KiB cap) and `timeout` (30 s) middlewares are now wired on every tool call. Logger construction in `cmd/serve.go` switches to `mcp-toolkit/logging.New` (JSON in-cluster auto-detect, text outside). `serviceName` is now an ldflags-injectable `var` in `cmd/root.go` (default `"mcp-kubernetes"`) — same pattern as mcp-template / mcp-prometheus, so rebrands flip a build flag instead of patching source. Health endpoints, OTel tracing init, OAuth-wrapped HTTP server, and the existing IP / token / email redaction helpers are intentionally untouched in this PR — they have wider blast radius and will land in follow-ups.
