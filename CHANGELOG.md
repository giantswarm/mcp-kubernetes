...
- `TRUSTED_ISSUERS` environment variable (JSON array of `{issuer, jwksURL, allowedAudiences, allowPrivateIPJWKS}` objects) to accept Bearer JWTs issued by external identity providers at the `/mcp` endpoint. Enables M2M flows where a machine identity provider such as muster issues access tokens directly to autonomous agents.
