...
Downstream OAuth (--downstream-oauth):
  ...
  Requires the Kubernetes cluster to be configured for OIDC authentication.`,
	...
	cmd.Flags().StringVar(&dexCAFile, "dex-ca-file", "", "Path to CA certificate file for Dex TLS verification. Applies to the Dex provider client AND, when --sso-allow-private-ips is set, the SSO forwarded-ID-token JWKS validation (the CA is also installed on http.DefaultTransport). (optional, can also be set via DEX_CA_FILE env var)")
