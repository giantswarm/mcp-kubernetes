...
| **mcp-kubernetes** | Maps the human `sub` → `Impersonate-User` and the acting agent → `Impersonate-Extra-actor`. Accepts any validated trusted-issuer actor; the impersonated human's Kubernetes RBAC governs access. |
...
identity from the subject. The human subject and the actor arrive from the broker.
...
4. Sets `Impersonate-User` (the human), `Impersonate-Group` (`system:authenticated`),
   and `Impersonate-Extra-{issuer,agent,actor}` headers on every downstream
   Kubernetes API call.
...
Multiple entries may share the same issuer URL; entries with `allowedClaims` are
tried first (subject-pattern match), entries without are the fallback.
...
- `impersonate userextras/{agent,issuer,actor}` — `mcp-kubernetes`, every
  configured issuer, and any actor (unrestricted)
